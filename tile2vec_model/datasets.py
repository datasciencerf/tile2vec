'''
This code is modified from https://github.com/ermongroup/tile2vec/blob/master/src/datasets.py
'''
...
class TileTripletsDataset(Dataset):
    ...
    def __init__(self, tile_dir, triplet_fp, transform=None, n_triplets=None,
                 pairs_only=True):
        ...
        with open(triplet_fp, 'rb') as handle:
            self.triplets = list(pickle.load(handle))
    ...
    def __getitem__(self, idx):
        id_a, id_n, id_d = self.triplets[idx]
        a = np.load(os.path.join(self.tile_dir, 'tile_{}.npy'.format(id_a)))
        n = np.load(os.path.join(self.tile_dir, 'tile_{}.npy'.format(id_n)))
        d = np.load(os.path.join(self.tile_dir, 'tile_{}.npy'.format(id_d)))
        # if self.pairs_only:
...
class ClipAndScale(object):
    ...
    def __init__(self, img_type):
        assert img_type in ['naip', 'rgb', 'landsat', 'airbus']
...
def triplet_dataloader(img_type, tile_dir, triplet_dir, bands=5, augment=False,
                       batch_size=4, shuffle=True, num_workers=4, n_triplets=None,
                       pairs_only=False):
    """
    Returns a DataLoader with either airbus (RGB/IR/EVI), NAIP (RGB/IR), RGB, or Landsat tiles.
    Turn shuffle to False for producing embeddings that correspond to original
    tiles.
    """
    assert img_type in {'landsat', 'rgb', 'naip', 'airbus'}
    transform_list = []
    ...
    dataset = TileTripletsDataset(tile_dir, triplet_dir, transform=transform,
